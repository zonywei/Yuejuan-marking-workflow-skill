...
```text
题目 + 参考答案 -> 评分标准 -> 提示词包 -> 人工智能辅助草评分 -> 复核队列 -> 对账 -> 教师阅卷报告
```

> 本项目不是让人工智能代替教师判分，而是帮助教师把阅卷流程做得更稳定、可复核、可追踪。
...
| 输出 | 评分标准、阅卷提示词、结构化草评分、低置信度复核列表、教师阅卷报告 |
| 价值 | 提高评分一致性，保留证据，暴露边界卷，支持提交后对账 |
| 不做什么 | 不做无人阅卷，不绕过验证码或权限限制，不保存真实学生隐私数据 |

```mermaid
flowchart LR
    A[题目 + 参考答案 + 分值] --> B[评分标准 / 得分点]
    B --> C[提示词包]
    C --> D[批量人工智能辅助草评分]
    D --> E[复核队列]
    E --> F[对账记录]
...
- 你是教师，想更安全地把人工智能用于主观题阅卷。
- 你在做智能体技能，需要一个真实教育工作流样例。
- 你在评估人工智能辅助评分，但不想做黑箱自动判分。
...
这是一个面向教师、教研团队、学校和教育公司的通用智能体阅卷工作流技能，用于辅助主观题阅卷、评分标准整理、批量评分、复核对账和教师报告生成。
...
- 人工智能辅助评分：要求结构化输出每小问得分、总分、理由、置信度和复核标记。
...
| Value | More consistent scoring, preserved evidence, reviewable edge cases, auditable submission flow |
| What it is not | Unattended auto-grading, CAPTCHA bypass, permission bypass, or storage for real student data |

```mermaid
flowchart LR
    A[Question + Answer Key + Score Allocation] --> B[Rubric / Scoring Atoms]
    B --> C[Prompt Pack]
    C --> D[Batch AI-assisted Draft Scoring]
    D --> E[Review Queue]
...
- You are a teacher and want a safer way to use AI for subjective marking.
- You are building an agent skill and need a real education workflow example.
- You are evaluating AI-assisted scoring but do not want black-box auto-grading.
...
The goal is not unattended grading. The goal is a stable workflow that makes teacher review faster, safer, and more traceable.
...
- Teachers who grade large volumes of subjective questions and want to reduce repeated work while preserving teacher confirmation and review.
...
Yuejuan Marking Workflow Skill is a general agent skill for teachers, teaching teams, schools, and education companies. It helps AI agents support subjective-question grading, rubric preparation, batch scoring, reconciliation, and teacher-facing report generation.
...
```text
A general subjective-question marking workflow for teachers and education teams: prepare rubrics, assist batch scoring, reconcile results, and generate teaching feedback reports.
```
